...
int main(int argc, const char** argv) {
  std::string option;
	auto cli = lyra::cli();
	cli.add_argument(
			lyra::opt(option, "running_option")
			.name("-o")
			.name("--opt")
  ...
  if (option == "sir") {
    ...
    cli.add_argument(lyra::opt(beta, "beta")
		    .name("-b").name("--beta").help("Set the beta value"));
    cli.add_argument(lyra::opt(gamma, "gamma")
		    .name("-g").name("--gamma").help("Set the gamma value"));
    cli.add_argument(lyra::opt(S, "susceptibles")
		    .name("-S").name("--susceptibles").help("Set the number of susceptibles at the start of the pandemic"));
    cli.add_argument(lyra::opt(I, "infected")
		    .name("-I").name("--infected").help("Set the number of infected at the start of the pandemic"));
    cli.add_argument(lyra::opt(R, "removed")
		    .name("-R").name("--removed").help("Set the number of removed at the start of the pandemic"));
    cli.add_argument(lyra::opt(duration, "duration")
		    .name("-d").name("--duration").help("Set the duration of the simulation"));
    ...
    cli.add_argument(lyra::opt(data_file, "data_file")
		    .name("-f").name("--file").help("Select the file containing the data to fit"));
