...
class Pandemy {
 ...
 public:
  Pandemy(int const people = 0, State const& state = {0, 0, 0}, Virus const& virus = {0.0, 0.0}) 
      : people{people}, state{state}, virus{virus} {
    if (state.S < 0 || state.I < 0 || state.R < 0) {
      throw std::range_error("Insert positive values!");
    } else if (people != state.S + state.I + state.R) {
      throw std::range_error("It must be N = S + I + R");
    } else if (virus.beta < 0. || virus.gamma < 0.) {
      throw std::range_error("Insert positive values!");
    }
  }
